...
input double                     target_percentage = 1000;       //Target Percentage
...
input ENUM_TIMEFRAMES            tradingTf = PERIOD_M15 ;
...
input int                        ma_1buy_level = 50 ;
input int                        ma_1sell_level = 50 ;
input int                        ma_2period = 2 ;
input int                        ma_2shift = -1 ;
input int                        ma_2buy_level = 40 ;
input int                        ma_2sell_level = 60 ;
input int                        envelope_priod = 3 ;
input double                     envelope_deviation = 0.2 ;
input double                     envelope_buy_level = 40 ;
input double                     envelope_sell_level = 60 ;
            ...
            for(int i = ObjectsTotal(0,0,OBJ_HLINE)-1; i >= 0; i--)
              {
               ...
               if(StringFind(name,"Resistencia") > -1)
                 {
                  datetime time = TimeCurrent();
                  double price =  ObjectGetDouble(0, name, OBJPROP_PRICE);
                  for(int j = 1; j <= 3; j++)
                    {
                     double candle_high = bar[j].high;   // get the high of the i-th previous candle
                     if(candle_high >= price)  // compare with the given price
                       {
                        Print("In Sell Mode");
                        trade.Sell(
                           lotSize,
               ...
               if(StringFind(name,"Soporte") > -1)
                 {
                  ...
                  for(int j = 1; j <= 3; j++)
                    {
                     double candle_low = bar[j].low;   // get the high of the i-th previous candle
                     if(candle_low <= price)  // compare with the given price
                       {
                        Print("In Buy Mode");
                        trade.Buy(
                           lotSize,
